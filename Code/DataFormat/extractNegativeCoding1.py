######################################################################################################################
# Script to extract fragments of coding regions that start from the TSS (translation start site).
##########
# Input:
...
##########
# Output:
# 1. cod1.csv: The csv file containing the extracted sequences.
#####################################################################################################################
...
outputFile = open(outputLocation+"cod1.csv","w")
...
while(seqCount < quantSeq):
    ...
    seq = codingList.pop(rSeq)
    if(len(seq) < seqSize): continue
    rPos = 0 # Always from the start of the coding regions
